...
“题目录入台”是题库的附属网站。它使用 GitHub OAuth 登录，只允许 `config/submitters.json` 白名单中且拥有仓库 Write 权限的账号新增或修改题目。一次提交会创建独立分支和 Pull Request，不会自动合并，也不会在后端运行 C++ 代码。
...
- 可以修改来源、平台题号和英文短名；由此产生的路径变化会自动迁移目录。
- 可以新增、修改或删除测试，但至少保留一组成对的输入输出。
- 提交前会按字段和行显示修改差异，并要求填写修改说明。
- 如果题目在编辑期间发生变化，提交会被拒绝，必须重新载入。
- 同一道题已有待审核的修改 PR 时，不允许重复创建。
- 编辑模式不提供删除整道题的功能。
...
- 整个请求最多 10MB。
- 后端只允许写入题目结构中的固定文件路径；目录迁移时保留未知附加文件。
- 后端不执行提交的代码。
- 同一来源和题号以及同名题目目录会被拒绝。
- 修改请求会校验原题目录版本，防止覆盖其他人刚刚提交的变化。
